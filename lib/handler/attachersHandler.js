...
function insertIntoGrid(newElement, host, grid) {
  ...
  // If host not yet placed, start a new row with host then place boundary below-right
  if (row < 0 || col < 0) {
    grid.add(host);
    const [hRow, hCol] = grid.find(host);
    grid.add(newElement, [hRow + 1, hCol + 1]);
    return;
  }

  // Grid is occupied
  if ((grid.get && grid.get(row + 1, col)) || (grid.get && grid.get(row + 1, col + 1))) {
    grid.createRow(row);
  }
