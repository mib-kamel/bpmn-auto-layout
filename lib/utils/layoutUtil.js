...
export function connectElements(source, target, layoutGrid) {
  ...
  if (!sourceDi || !targetDi || !sourceDi.get || !targetDi.get) {
    // cannot compute connection yet (likely missing DI)
    return [];
  }
  ...
  if (!source.gridPosition || !target.gridPosition) {
    return [getDockingPoint(sourceMid, sourceBounds, 'r'), getDockingPoint(targetMid, targetBounds, 'l')];
  }
